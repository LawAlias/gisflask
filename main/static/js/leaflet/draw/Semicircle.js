...
(function (factory) {
  ...
  var semicircle = {
    ...
    startAngle: function () {
      /*edit by wangshilian 2018/05/08 渲染时调用*/
      ...
      var start = this.options.startAngle-90>0?this.options.startAngle-90:this.options.startAngle-90+360;
      return start*DEG_TO_RAD;
    },
    stopAngle: function () {
      /*edit by wangshilian 2018/05/08 渲染时调用*/
      /*if (this.options.startAngle < this.options.stopAngle) {
      ...
      var stop = this.options.startAngle<this.options.stopAngle?this.options.stopAngle-90:this.options.stopAngle-90+360;
      return stop * DEG_TO_RAD;
    },
  ...
  L.SVG.include({
    _updateCircle: function (layer) {
        ...
        var p = layer._map.latLngToLayerPoint(layer._latlng),
            ...
            startAngle =layer.options.startAngle>0?layer.options.startAngle:layer.options.startAngle+360,
            stopAngle = layer.options.stopAngle<startAngle?layer.options.stopAngle+360:layer.options.stopAngle;
        
        var largeArc = (stopAngle - startAngle >= 180) ? '1' : '0';
        var d = 'M' + p.x + ',' + p.y +
  ...
  L.Canvas.include({
    _updateCircle: function (layer) {
      if (!this._drawing || layer._empty()) { return; }/*added by wangshilian 2018/05/07 解决扇形编辑时拖拽闪烁的问题*/
      // If we want a circle, we use the original function
      if (!(layer instanceof L.SemiCircle || layer instanceof L.SemiCircleMarker) ||
        !layer.isSemicircle()) {
